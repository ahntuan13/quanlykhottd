...
ACT['ca-save']=()=>{
  if(!CA)return;const cat=by(db.categories,CA.id);let moved=0,released=0;
  const ok=transact(()=>{
    const un=[...CA.init].some(id=>!CA.sel.has(id))?uncategorized():null;
    db.items.forEach(i=>{
      if(CA.sel.has(i.id)&&i.categoryId!==CA.id){i.categoryId=CA.id;moved++}
      else if(!CA.sel.has(i.id)&&i.categoryId===CA.id){i.categoryId=un.id;released++}
    });
  });
  if(ok){CA=null;done(`Đã lưu nhóm ${cat.name}: thêm ${moved} mặt hàng${released?`, đưa ${released} mặt hàng về Chưa phân loại`:''}`)}
};
