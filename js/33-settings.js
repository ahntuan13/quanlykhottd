...
const CRUD={
  supplier:{col:'suppliers',pre:'s',t:'Nhà cung cấp',fields:[['code','Mã NCC'],['name','Tên nhà cung cấp',1],['taxId','Mã số thuế'],['bankAccount','Số tài khoản'],['contact','Người liên hệ'],['phone','Điện thoại'],['email','Email'],['address','Địa chỉ']],unique:'code',upload:['up-ncc','Upload file NCC'],
...
PAGES['set/system']={t:'Sao lưu & Hệ thống',
  r(){
    ...
    ${card('Thông tin công ty (hiển thị trên phiếu xuất, vì mình là bên xuất/bán hàng)',`<form data-submit="company"><div class="fg">${inp('name','Tên công ty',db.company.name,{full:1,req:1})}${inp('taxId','Mã số thuế',db.company.taxId||'')}${inp('bankAccount','Số tài khoản',db.company.bankAccount||'')}${inp('address','Địa chỉ',db.company.address,{full:1})}${inp('phone','Điện thoại',db.company.phone)}${inp('defaultVatRate','Thuế suất GTGT mặc định (%)',fmtPrice(db.company.defaultVatRate||0),{attrs:'inputmode="decimal" data-num="money"'})}<div class="f full" style="justify-content:flex-end"><button class="btn primary" ${adm?'':'disabled'}>Lưu thông tin</button></div></div></form><p class="note">Phiếu <b>nhập</b> hiển thị mã số thuế / số tài khoản của <b>Nhà cung cấp</b> (Thông tin → Supplier) thay vì của công ty, vì đó là bên xuất hóa đơn cho mình.</p>`)}
...
SUB.company=form=>{if(!can('admin'))return toast('Chỉ quản trị viên được sửa.','error');const d=fd(form);db.company={name:d.name.trim(),taxId:(d.taxId||'').trim(),bankAccount:(d.bankAccount||'').trim(),address:d.address.trim(),phone:d.phone.trim(),defaultVatRate:r2(numVN(d.defaultVatRate))||0};save();render(true);toast('Đã lưu')};
