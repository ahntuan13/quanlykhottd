...
1. Giải nén file zip, mở thư mục `quanlykho-ttd`, chọn **toàn bộ** nội dung bên trong (Ctrl+A).
2. Vào repository → **Add file → Upload files** → kéo tất cả vào (kéo cả các thư mục `css`, `js`, `assets`, `config`) → **Commit changes**.
3. Bản cũ chỉ có mỗi `index.html`: file mới sẽ ghi đè lên nó.
4. Settings → Pages → Deploy from a branch → `main` / `(root)` (chỉ làm một lần).
5. Sau ~1 phút mở trang và bấm Ctrl+Shift+R.
...
5. **Databases & Storage → Firestore** → Create database → **Standard edition** → vị trí `asia-southeast1` → **Production mode**.
...
8. Mở app → **Thiết lập lần đầu** → tạo quản trị viên → Settings → User / Permission để tạo tài khoản cho người khác.
